...
%% Chaudhuri Model as a function for PSO
...
v_E = zeros([29 length(time)]);                                              % Starting firing rate for excitatory, rows are regions, columns are time
v_I = zeros([29 length(time)]);
zerovec = zeros([29 1]);
